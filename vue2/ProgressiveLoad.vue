...
<template>
  <div class='ProgressiveLoad'>
    ...
    <template v-if="list">
      ...
      <no-data v-if="list.length===0 && listStatus==='加载成功'">
    ...
    tryQuery(reqNormalParams={}){
      ...
      return this.req({
        ...
        .then((resData) => {
          ...
          const total=this.getTotal(resData)
          ...
          const fragment=this.getFrontEndData(resData)
          if(list===undefined||list===null/* 小程序里undefined传进组件会变成null */){
            list=[]
          }
          for(let i=0;i<fragment.length;i++){
            // console.log('list',list)
            this.$set(list,reqIdx*this.fragmentLen+i,fragment[i])
          }
